...
// Checks if a PressurePlate has been assigned
...
// Gets the total mass of objects overlapping
...
void URotateDoor::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	// Checks if trigger conditions are true to open door.
...
void URotateDoor::CloseDoor(float DeltaTime) {
	...
	CurrentYaw = FMath::FInterpConstantTo(CurrentYaw, InitialYaw, DeltaTime, DoorCloseSpeed); //Door slam shut quicker and at linear speed
	FRotator DoorRotation = GetOwner()->GetActorRotation();
	DoorRotation.Yaw = CurrentYaw;
	GetOwner()->SetActorRotation(DoorRotation);
	if (!AudioComponent) { 
